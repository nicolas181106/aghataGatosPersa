...
Actualmente todos nuestros gatitos europeos están adoptados, pero puedes explorar nuestras camadas anteriores o dejarnos tu correo para avisarte cuando tengamos nuevos gatitos.
...
Cliqueame
</span> </a> <div class="flex flex-col text-left text-lg lg:text-xl"> <p><strong>Nombre:</strong> ${gato.nombre}</p> <p><strong>Género:</strong> ${gato.genero}</p> <p><strong>Edad:</strong> <span class="edad-gatito"${addAttribute(gato.edad.toISOString(), "data-nacimiento")}>Calculando...</span></p> <div class="flex gap-4 mt-6"> <a href="/contacto" class="text-sm sm:text-base inline-block hover:bg-[#FF6B6B] border-2 border-[#FF6B6B] text-[#FF6B6B] hover:text-white font-semibold py-2 px-4 rounded-full transition duration-300 shadow">
Adoptar gatito
...
const $$file = "C:/Users/Nicolás/Desktop/TODO/SOFTWARES/PROGRAMACIÓN/VISUAL STUDIO CODE/Mis páginas web/agathaGatos/src/pages/persa-europeo.astro";
